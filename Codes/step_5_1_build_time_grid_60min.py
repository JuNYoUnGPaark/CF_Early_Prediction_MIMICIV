...
# ============================================================
# 5-1. Build Patient Time Grid
# ============================================================
#
# 현재 MIMIC-IV 구현 기준:
#   - 실제 preprocessing grid: 60 min
#   - 비교용 audit: 같은 stay window에서 5-min grid였다면
#                  point가 몇 개 생기는지만 계산
#
# grid 범위:
#   start = ICU stay 내 first Heart Rate measurement
#   end   = min(
#               ICU stay 내 last Heart Rate measurement,
#               ICU outtime,
#               ICU intime + 28 days
#           )
#
# 이 단계에서는 timestamp만 만든다.
# measurement 배치 / missingness / imputation은 아직 하지 않는다.
# ============================================================
...
def build_time_grid(
    ...
    if missing:
        raise ValueError(
            f"stays에 필요한 column이 없습니다: {missing}"
        )
    ...
    # --------------------------------------------------------
    # 7. 같은 stay window를 5-min으로 만들었다면
    #    몇 point인지 계산만 함
    # --------------------------------------------------------
