...
# ============================================================
# 4-1A. Merge Confirmed Non-pharmaceutical Concepts
# ============================================================
#
# 목적
# ------------------------------------------------------------
# 현재까지 MIMIC-IV mapping이 명확하게 확인된
# physiology / lab ITEMID만 canonical variable로 통합한다.
#
# 이 단계에서 하는 일:
#   1) chartevents / labevents에서 확정 ITEMID만 선택
#   2) 동일 clinical concept 이름 부여
#   3) labevents에 ICU stay_id 연결
#   4) source_table / source_itemid 보존
#
# 이 단계에서 하지 않는 일:
#   - 같은 시각 measurement median 계산
#   - adaptive imputation
#   - 5-minute grid
#   - pharmaceutical variable 처리
#   - fuzzy / synonym 자동 mapping
#
# 즉:
#   "어떤 raw ITEMID들이 같은 clinical variable인가?"
#   만 확정하는 단계이다.
# ============================================================
...
def merge_nonpharma_concepts(
    ...
    """
    Confirmed physiology / lab ITEMIDs를 canonical variable로 통합한다.

    Returns
    -------
    events : pd.DataFrame
        stay_id, charttime, variable, valuenum,
        source_table, source_itemid, is_cf_map_source

    variable_report : pd.DataFrame
        canonical variable별 rows / stays / source ITEMID 수

    mapping_report : pd.DataFrame
        실제 데이터에 등장한 source ITEMID → canonical variable mapping
    ...
    missing = required_stays - set(stays.columns)
    if missing:
        raise ValueError(f"stays에 필요한 column이 없습니다: {missing}")
    ...
    #   "in mg/dl -> convert to mmol/l by * 1/18."
    #
    # Supplementary default value for Blood Glucose is 5,
    # which is on the mmol/L numeric scale used by the author pipeline.
    # --------------------------------------------------------
    ...
    #
    #   same hadm_id
    #   AND ICU intime <= charttime <= ICU outtime
    #
    # 조건으로 ICU stay_id를 연결한다.
    # ========================================================
    ...
    # ========================================================
    # 4. 하나의 lab row가 둘 이상의 ICU stay에 들어가는지 확인
    # ========================================================
